...
#        'lcds1frames', 'lcds2frames', 'visibleFrames', 'tight', 'layoutNr', 'swaplcds']
    ...
    def __init__(self, parent:'QWidget', aw:'ApplicationWindow') -> None:
        ...
        self.visibleFrames:list[bool] = [] # visibility flags in display order for all lcd frames
...
#        self.setWindowModality(Qt.WindowModality.NonModal) # this seems not to be effective on RPi bookworm thus we set the parent to None
    ...
    def hideAllEmptyLabels(self) -> None:
        if all(ll.text().strip() == '' for ll in (self.lcds1labelsLower + self.lcds2labelsLower)):
    ...
    def reLayout(self, n:int|None = None) -> None:
        if self.layoutNr != n:
    ...
    def updateVisibilities(self, l1:list[bool], l2:list[bool]) -> None:
        self.visibleFrames = [val for pair in zip(l1, l2, strict=True) for val in pair]
    ...
    def updateValues(self, values1:list[str|None], values2:list[str|None], **kwargs:str|None) -> None:
        del kwargs
    ...
    def updateLabels(self, lowerlabels1:list[str|None],
            lowerlabels2:list[str|None],
            upperlabels1:list[str|None],
            upperlabels2:list[str|None],
            **kwargs:str) -> None:
        del kwargs
...
class LargePhasesLCDs(LargeLCDs):
    ...
    @staticmethod
    def formatLabel(ll:str|None) -> str|None:
        if ll is None:
            return None
        return f'<b>{ll}</b>'
    ...
    def updatePhasesLabels(self, labels:list[str|None]) -> None:
        # don't update None values
