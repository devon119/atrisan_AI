...
def _compute_rule_advice(bt: float, ror_bt: float,
    ...
    if 'DROP' in trigger or '下豆' in trigger:
        return (f'現況：出豆了，{mins}:{secs:02d}，BT {bt:.1f}°C——這爐結束\n'
                f'操作：冷卻盤風扇全開，豆子均勻攤開散熱；聞香氣看豆色，記錄感受\n'
                f'▶ 繼續烘（鍋間流程）確認清單：\n'
                f'  1. 冷卻盤豆子冷透了沒？先收好再投下一爐\n'
                f'  2. 銀皮槽清一下，積太多會影響排煙\n'
                f'  3. 鍋爐溫度確認——是否已穩定在目標入豆溫？\n'
                f'  4. 下一批生豆秤好了嗎？豆種/克重確認\n'
                f'  5. 這爐數據記錄好（豆色、香氣、DTR），決定下一爐怎麼調\n'
                f'▶ 結束收工確認清單：\n'
                f'  1. 冷卻盤豆子冷透後收好\n'
                f'  2. 火力歸零，鍋爐空轉降溫（建議10分鐘以上）\n'
                f'  3. 銀皮槽清理，冷卻盤清潔\n'
                f'  4. 鍋爐降溫後關火、關風門、關機\n'
                f'  5. 記錄這爐數據，Artisan存檔後關閉\n'
                f'預期：AI完整評估報告馬上來，對照豆色香氣感受一起看')
